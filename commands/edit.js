...
module.exports.run = async (client, pf, message, args, nope, info, okay, what, warning, manager) => {
    ...
    if (!args[1]) {
        let nembed = new Discord.MessageEmbed().setColor('E93C21').setAuthor(message.author.tag, message.author.avatarURL(), `https://github.com/Ezzud/tadaa`).setDescription(`${nope} *Veuillez renseigner l'option à changer* (gagnants, prix)`).setFooter(`TADAA | créé par ezzud`)
        message.channel.send(nembed)
        return;
    }
    if (args[1] === 'gagnants') {
        if (!args[2]) {
            let nuembed = new Discord.MessageEmbed().setColor('E93C21').setAuthor(message.author.tag, message.author.avatarURL(), `https://github.com/Ezzud/tadaa`).setDescription(`${nope} *Veuillez renseigner un nombre de gagnants*`).setFooter(`TADAA | créé par ezzud`)
        ...
        mess = mess.toString()
        if (mess === 'NaN') {
            let nueembed = new Discord.MessageEmbed().setColor('E93C21').setAuthor(message.author.tag, message.author.avatarURL(), `https://github.com/Ezzud/tadaa`).setDescription(`${nope} *Vous devez entrer un nombre*`).setFooter(`TADAA | créé par ezzud`)
            ...
            manager.edit(args[0], {
                ...
                message.react('✅');
                let yembed = new Discord.MessageEmbed().setColor('24E921').setAuthor(message.author.tag, message.author.avatarURL(), `https://github.com/Ezzud/tadaa`).setDescription(`${okay} *La modification sera effectué à la prochaine mise à jour du message*`).setFooter(`TADAA | créé par ezzud`)
        ...
        if (!args[2]) {
            let noaembed = new Discord.MessageEmbed().setColor('E93C21').setAuthor(message.author.tag, message.author.avatarURL(), `https://github.com/Ezzud/tadaa`).setDescription(`${nope} *Veuillez renseigner le nouveau prix*`).setFooter(`TADAA | créé par ezzud`)
        ...
        let noedmbed = new Discord.MessageEmbed().setColor('E93C21').setAuthor(message.author.tag, message.author.avatarURL(), `https://github.com/Ezzud/tadaa`).setDescription(`${nope} *Cette option n'est pas reconnue, veuillez entrer une option valide* (gagnants, prix)`).setFooter(`TADAA | créé par ezzud`)
