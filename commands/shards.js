...
module.exports.run = async (client, pf, message, args, manager,json,lang) => {
    ...
    let values = await client.shard.broadcastEval(`[ this.shard.id, this.guilds.cache.size ]`);
    ...
    .setColor("#337dff")
    .addField(`${client.online} Etat:`, `Nombre de shards actifs: \`${values}\`/\`${client.shard.count}\``);
    let i = 0
    for(i in guildSize) {
        if(guildSize[i]) {
