...
client.on('messageReactionAdd', async (messageReaction, user) => {
    ...
    let gw;
        gw = manager.giveaways.filter((g) => g.guildID === messageReaction.message.guild.id);
        gw = manager.giveaways.filter((g) => g.ended === false);
    if(!gw) return;
    let checkRole = false
    gw.forEach(give => {
    ...
    if(checkRole === true) {
        ...
        let gw2;
            gw2 = manager.giveaways.filter((g) => g.messageID === messageReaction.message.id);
        if(!gw2) return;
        ...
        if(guild.member(user.id).roles.cache.find(x => x.id === role.id)) {
           ...
           try {
                ...
                .setAuthor(`Erreur!`, icon_url='https://cdn.discordapp.com/attachments/682274736306126925/740643196878454834/1596653488174.png')
                .setColor('#ED3016')
                .addField(`\u200B`, `Vous ne pouvez pas participer à ce giveaway car vous ne possédez pas le rôle \`${role.name}\` \n[Clique ici](https://discordapp.com/channels/${messageReaction.message.guild.id}/${messageReaction.message.channel.id}/${messageReaction.message.id}) pour accéder au message`)
