...
values.forEach((value) => {
    ...
    prereq = prereq.reduce((p, n) => p + n, 0);
        ...
        await client.user.setPresence({ activity: { name: `@TADAA | Shard ${count}/${client.shard.count} | ${prereq} serveurs | v${json.version}` }, status: 'dnd' })
setInterval(async () => {
    let count2 = 0;
    ...
    req = req.reduce((p, n) => p + n, 0);
        ...
        await client.user.setPresence({ activity: { name: `@TADAA | Shard ${count2}/${client.shard.count} | ${req} serveurs | v${json.version}` }, status: 'dnd' })
