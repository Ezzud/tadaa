...
const manager = new GiveawaysManager(client, {
  ...
  if(command === 'edit' || command === 'delete' || command === 'end' || command === 'start') {
    if(gwDelay.has(message.author.id)) {
        ...
        .setColor('E3260F')
        .addField(`\u200B`, `Veuillez patienter *10 secondes* avant chaque utilisation de cette commande`)
    ...
    if(delay.has(message.author.id)) {
        ...
        .setAuthor(message.author.tag, message.author.avatarURL(), `https://github.com/Ezzud/tadaa`)
        .setColor('E3260F')
        .addField(`\u200B`, `Veuillez patienter *3 secondes* avant chaque utilisation de commande`)
        .setFooter(`TADAA | créé par ezzud`)
        message.channel.send(embed)
        return;
    } else {
